...
cocktailApp.init = () => {
    if (mediaQuery.matches) {
        // eventListener so that when users click outside of the search bar it closes: 
...
// establishing media-query for tablet/mobile (below 700px):
...
// method to soften elements exit off page:
...
// method to soften the alcohol elements exit off page:
...
// method to hide recipe box: 
cocktailApp.hideRecipe = function() {
...
// method to get random drink:
cocktailApp.randomizeDrink = function (jsonInfo) {
    cocktailApp.drinksArray = [];
    // create an array from returned json
    cocktailApp.drinksArray = Array.from(jsonInfo);
...
// method to take user input and return drink:
cocktailApp.chooseDrink = function (event) {
    event.preventDefault();
    cocktailApp.hideRecipe();
        ...
        fetch(cocktailApp.alcoholUrl)
            .then((response) => {
                ...
                if (jsonResult.drinks === "None Found") {
                    ...
                    // get random drink -> print drink name & image preview to page:
                    cocktailApp.showDrinkPreview(cocktailApp.randomizeDrink(jsonResult.drinks));
                }
            });
...
cocktailApp.searchForm.addEventListener("submit", function (e) {
    ...
    fetch(cocktailApp.searchUrl).then((response) => {
        ...
        if (jsonResult.drinks === null) {
            ...
            cocktailApp.fadeOut(cocktailApp.buttonContainer, 1);
            // make an array from the returned object so that we can loop through them:
            const newArray = Object.keys(drink);
            // Reset the ingredients and measurements arrays
            cocktailApp.ingredients = [];
            cocktailApp.measurements = [];
            // establish ingredients and measurements for current drink:
            cocktailApp.parseArray(newArray, cocktailApp.ingredients, "strIngredient", drink);
            cocktailApp.parseArray(newArray, cocktailApp.measurements, "strMeasure", drink);
            // reset recipe container content:
            cocktailApp.resetRecipeContainer();
            // print instructions to page
            document.querySelector('.instructionList').textContent = drink.strInstructions;
            // print ingredients & measurements to page
            const ingredientList = document.querySelector(".ingredientList");
            cocktailApp.ingredients.forEach((ingredient, index) => {
                const measurement = cocktailApp.measurements[index];
